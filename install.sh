...
if ! command -v python3 >/dev/null 2>&1; then
  echo "Python 3 is required to register the TUI plugin without replacing existing configuration." >&2
  exit 1
fi
...
if [[ -f "${opencode_config_dir}/tui.jsonc" ]]; then
  readonly tui_config_path="${opencode_config_dir}/tui.jsonc"
else
  readonly tui_config_path="${opencode_config_dir}/tui.json"
fi

python3 - "${tui_config_path}" "${plugin_path}" <<'PY'
import json
import os
import re
import sys
...
def register(source: str, value: str):
    ...
    if not tokens or tokens[0][0] != "{" or matching_close(tokens, 0) != len(tokens) - 1:
        raise ValueError("the TUI configuration must contain one top-level object")
    ...
    while index < root_close:
        ...
        if token[0] in ("{", "["):
            ...
            depth -= 1
        elif depth == 0 and token[0] == "string" and token[3] == "plugin":
            if index + 2 >= root_close or tokens[index + 1][0] != ":" or tokens[index + 2][0] != "[":
                raise ValueError('the top-level "plugin" setting must be an array')
